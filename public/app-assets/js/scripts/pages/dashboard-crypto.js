...
$(window).on("load", function(){
    ...
    require(
        ...
        function (ec) {
            ...
            $.ajax({
                type: "GET",
                url: "https://min-api.cryptocompare.com/data/histoday?fsym=BTC&tsym=USD&limit=60&aggregate=1&e=CCCAGG",
                dataType: "json",
                async: false,
            ...
            for(var i=0; i<result.Data.length; i++){
                ...
                var unixtimestamp = result.Data[i].time;
                ...
                datetime.push(convdataTime);

                var close = result.Data[i].close;
                var open = result.Data[i].open;
                var high = result.Data[i].high;
                var low = result.Data[i].low;

                var items = [open, close, high, low]; 
            ...
            datetime.reverse();
            iterations.reverse();
            $('#last_price').html(result.Data[60].close);
            var daily_change = result.Data[60].close - result.Data[59].close;
            $('#daily_change').html(daily_change);
